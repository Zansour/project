...
# date time feature 생성 - [연도, 월, 일, 요일, 시간]
...
# =============================================================================
# [건물 유형]별 모델링을 선택한 이유? EDA와 실효성
# =============================================================================
# ( EDA ) [건물 유형]별 일별 전력 소비량
# - 데이터 센터와 대학교는 유형별 모델링 의미있음.
imsi = train_df.groupby(['building_number', 'month', 'day', 'building_type'])[['power_consumption']].sum()
imsi.reset_index(inplace=True)
imsi2 = imsi.groupby(['building_type', 'month', 'day'])[['power_consumption']].mean()
imsi2.reset_index(inplace=True)

imsi2['month'] = [str(i) for i in imsi2['month']]
imsi2['day'] = [str(i).zfill(2) for i in imsi2['day']]
imsi2['date'] = imsi2['month'] + '/' + imsi2['day']

mean_power_by_date_building = imsi2.groupby(['date', 'building_type'])['power_consumption'].mean().reset_index()
pivot_df = mean_power_by_date_building.pivot(index='date', columns='building_type', values='power_consumption')

x_positions = list(range(0, 85, 3))
x_labels = [imsi2['date'][i] for i in x_positions]
...
# ( EDA ) 데이터 센터와 대학교 제외
# - [건물 유형]에 따라 평일/주말 전력 사용 추이가 다름. 
temp = train_df[(train_df['building_type'] != 'Data Center') & (train_df['building_type'] != 'University')]
imsi = temp.groupby(['building_number', 'month', 'day', 'building_type'])[['power_consumption']].sum()
imsi.reset_index(inplace=True)
imsi2 = imsi.groupby(['building_type', 'month', 'day'])[['power_consumption']].mean()
imsi2.reset_index(inplace=True)
...
# ( EDA ) [건물 유형]별 건물 번호 별 일 평균 전력량
# - 시간대별 사용량에 유사한 패턴을 보임을 확인
def typeHourConsumption(df: pd.DataFrame, bd_type: str):
    
    temp = df.loc[df['building_type'] == bd_type, :]
    pivot_df = temp.groupby(['hour', 'building_number'])['power_consumption'].mean().unstack()
    
    fig, ax = plt.subplots(figsize=(15, 10))
    plt.xticks(x_positions, x_labels)
    sns.lineplot(data=pivot_df)
    
    plt.title('Mean Power Consumption by Day and Building Type(' + bd_type + ')')
    plt.xlabel('Day')
    plt.ylabel('Mean Power Consumption')
    plt.legend(title='Building Type', loc='upper left')
...
for i in train_df['building_type'].unique() :
    typeHourConsumption(train_df, i)
...
# 8-2) 냉방면적 이상치 대체
# - 77, 80번 건물 냉방면적 1239로 현저히 낮음. 건물 유형이 지식산업센터인 건물의 냉방면적 평균 적용
...
# 10. 불필요한 변수 제거
train_df.drop(['num_date_time', 'hour', 'total_area', 'cooling_area'], axis = 1, inplace = True)
test_df.drop(['num_date_time', 'hour', 'total_area', 'cooling_area'], axis = 1, inplace = True)
...
for i in tqdm(np.arange(0, 12)):
    y = train_df.loc[train_df.building_type == i, 'power_consumption']
    x = train_df.loc[train_df.building_type == i, ].drop(['power_consumption', 'date_time','building_type'], axis=1)
    ...
    building = 'building_type : '+le.classes_[i]
    print(building + '|| SMAPE : {}'.format(SMAPE(y_test, pred)))
    preds = np.append(preds, pred)
    df = pd.concat([df, pd.DataFrame(params, index = [0])], axis = 0)

df.to_csv('xgb_hyperparameter_type.csv',index=False)
xgb_params = pd.read_csv('xgb_hyperparameter_type.csv')
...
for i in tqdm(range(12)):
    ...
    print("building_type {} || best score : {} || alpha : {}".format(i, score0, best_alpha))

no_df = pd.DataFrame({'score':smape_list})
plt.bar(np.arange(len(no_df))+1, no_df['score'])
plt.plot([1,12], [3, 3], color = 'red')

xgb_params['alpha'] = alpha_list
xgb_params['best_it'] = best_it
xgb_params.head()

xgb_params.to_csv('xgb_hyperparameter_type_final.csv', index=False)

# =============================================================================
# 최종 학습 및 predict(seed ensemble)
# =============================================================================
xgb_params = pd.read_csv('xgb_hyperparameter_type_final.csv')
xgb_params.head()
...
for i in tqdm(range(12)):
    ...
    for seed in [0,1,2,3,4,5]: # 각 시드별 예측
        y_train = train_df.loc[train_df.building_type == i, 'power_consumption']
        x_train, x_test = train_df.loc[train_df.building_type == i, ].drop(['power_consumption', 'date_time','building_type'], axis=1), test_df.loc[test_df.building_type == i, ].drop(['date_time','building_type'], axis=1)
...
submission.to_csv('submission_xgb_type.csv', index = False)
